        ...
        if(CONTENT_TYPE_FORM.equals(contentType)){
            ...
            for (String key : map.keySet()) {
                ...
                if(valueObj instanceof Collection || valueObj instanceof Map){
                    params.append(key).append("=").append(JSON.toJSONString(valueObj)).append("&");
                }else if(valueObj instanceof String
                        || valueObj instanceof Long
                        || valueObj instanceof Integer
                        || valueObj instanceof Float
                        || valueObj instanceof Charset
                        || valueObj instanceof Boolean ){

                    params.append(key).append("=").append(valueObj).append("&");
                }else{
                    params.append(key).append("=").append(JSON.toJSONString(valueObj)).append("&");
                }
            }
            ...
            se = new StringEntity(JSONObject.toJSONString(map), "UTF-8");
        }
...
//            httpclient.
     ...
     * @Description post请求
     */
    public static String post(String url, Map<String, Object> map,Map<String,String> header) throws IOException {
        HttpClient httpClient = HttpClients.createDefault();
        HttpPost http = new HttpPost(url);
        for(String key:header.keySet()){
//            System.out.println(key+"="+header.get(key));
        ...
        StringEntity se = new StringEntity(JSONObject.toJSONString(map),"GBK");
        http.setEntity(se);
        HttpResponse httpResponse = httpClient.execute(http);
        BufferedReader buffer = new BufferedReader(new InputStreamReader(httpResponse.getEntity().getContent(),"GBK"));
        StringBuffer stb = new StringBuffer();
        String line = null;
        while ((line = buffer.readLine()) != null) {
            stb.append(line);
